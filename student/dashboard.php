...
$courses = $db->select('student_course', ['course','done'], ['student' => $userid]);

//populate extra required data
foreach($courses as $key => $course) {
	$result = $db->select('courses', ['code','name','instructor'], ['courseid' => $courses[$key]['course']]);
	$courses[$key]['code'] = $result[0]['code'];
	$courses[$key]['name'] = $result[0]['name'];
	$courses[$key]['instructor'] = $result[0]['instructor'];
...
<div class="list-group">
  ...
  ?>
  <a <?php if($course['done']==0) { ?> href="<?php echo $config['url']['base_url'].$config['url']['feedback_form']."?courseid=".$course['course']; ?>" <?php } ?> class="list-group-item list-group-item-<?php if($course['done'] == 1) {echo 'success';} else {echo 'warning';} ?>">
  	  <div class="row">
        <div class="col-md-7">
