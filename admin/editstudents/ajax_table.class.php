...
require_once(dirname(dirname(__FILE__)) . '\editstudents\config.php');
...
class ajax_table {
  ...
  function getRecords(){
	$this->res = mysql_query("select * from students");
  ...
  function save($data){
	if(count($data)){
		...
		mysql_query("insert into students (".implode(",",array_keys($student_data)).") values ('".$values."')");
  ...
  function delete_record($id){
	 if($id){
		...
		mysql_query("delete from students where userid = $id limit 1");
		$res = mysql_affected_rows();
		//delete from student course linkage
		mysql_query("delete from student_course where student = $id limit 1");
		return $res;
	 }
  }	
  ...
  function update_record($data){
	if(count($data)){
		...
		$str = substr($str,0,-1);
		$sql = "update students set $str where userid = $id limit 1";
  ...
  function update_column($data){
	if(count($data)){
		$id = $data['rid'];
		unset($data['rid']);
		$sql = "update students set ".key($data)."='".$data[key($data)]."' where userid = $id limit 1";
		$res = mysql_query($sql);
