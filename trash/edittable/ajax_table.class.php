...
class ajax_table {
  ...
  function getRecords(){
	$this->res = mysql_query("select * from $this->db_table");
  ...
  function delete_record($id){
	 if($id){
		mysql_query("delete from $this->db_table where id = $id limit 1");
  ...
  function update_record($data){
	if(count($data)){
		...
		$str = substr($str,0,-1);
		$sql = "update $this->db_table set $str where id = $id limit 1";
  ...
  function update_column($data){
	if(count($data)){
		$id = $data['rid'];
		unset($data['rid']);
		$sql = "update $this->db_table set ".key($data)."='".$data[key($data)]."' where id = $id limit 1";
		$res = mysql_query($sql);
		if(mysql_affected_rows()) return $id;
		return 0;
